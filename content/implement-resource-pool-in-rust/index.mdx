...
The *PoolRequest* is a lightweight object that act as a request to the pool, it is Send + Sync + Clone, and also very cheap to clone across multiple threads.
It's retrieve fn will request the pool for a PoolResponse;

The PoolResponse; has implemented Deref&lt;T&gt; + DerefMut&lt;T&gt; so that it can be dereferenced into the resource.
...
### 2. Applying max_pool_size could result in a rapid increase in the number of resources.
   ...
   For example, if your Rest api most of the time only need to handle 10 concurrent requests and you set min_pool_size = 10, but suddenly receive 1,000 requests at once, your application will return to 10 concurrent requests after the surge, leaving 990 resources unused in the pool.
...
***Note: The number of resources in the pool will always be ≥ min_pool_size.***

### 3. What if requests exceed max_pool_size? Should we fail those requests?

   We can handle this by applying retrieving_timeout to PoolRequest. This allows the request to wait (within a specified timeout) for a resource to be released back into the pool, which can then be used for the current request.
...
<Lottie className='aspect-[1.4/1] my-4 rounded-lg' file='/dotlottie/resource-pool_-_full-flow.lottie'/>
...
Let's take a look on struct PoolItem

```rust
pub struct PoolItem<T> where T: Send + Sync + 'static
{
    // Put the resource on Heap could reduce memory usage.
    // For example if there are three variables with type Option<T>;,
...
The PoolAllocator&lt;T&gt; holds a Vector of PoolItem, which is wrapped inside an Arc to allow sharing the reference across different threads. We then wrap this with an RwLock, enabling multiple readers but only one writer at a time. Together, these wrappers allow us to share the PoolAllocator across threads without violating Rust's memory safety rules.
...
Additionally, this ensures that the PoolCleaner will be dropped when the allocator is dropped.
