...
type UsePromiseReturn<T extends string[], P extends any[], R> = {
  ...
  [K in keyof T as `set${Capitalize<T[K] & string>}`]: (
    value: P[K & number],
  ) => void;
} & {
  [K in keyof T as `set${Capitalize<T[K] & string>}State`]: (
    value: P[K & number],
  ) => void;
} & {
...
export function usePromise<T extends string[], P extends any[], R>(
  ...
  const paramRefs = useRef<P>(
    paramNames.length > 0
      ? (new Array(paramNames.length).fill(null) as P)
      : ([] as any),
  );
  const [paramStates, setParamStates] = useState<P>(
    paramNames.length > 0
      ? (new Array(paramNames.length).fill(null) as P)
      : ([] as any),
  );
  ...
  const setters = paramSetters.reduce(
    (acc, setter, index) => {
      const paramName = `set${capitalize(paramNames[index])}`;
      acc[paramName as `set${Capitalize<(typeof paramNames)[number]>}`] =
        setter as any;
      return acc;
    },
    {} as {
      [K in T[number] as `set${Capitalize<K>}`]: (value: P[number]) => void;
    },
  );

  const stateSetters = paramStateSetters.reduce(
    (acc, setter, index) => {
      const paramName = `set${capitalize(paramNames[index])}State`;
      acc[paramName as `set${Capitalize<(typeof paramNames)[number]>}State`] =
        setter as any;
      return acc;
    },
    {} as {
      [K in T[number] as `set${Capitalize<K>}State`]: (
        value: P[number],
      ) => void;
    },
  );

  const states = paramStates.reduce(
    (acc, state, index) => {
